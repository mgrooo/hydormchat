...
DEFAULT_QUICK_QUESTIONS_EN = [
    ...
    "When is the admission result announcement?",
    "When is the dorm fee payment period?",
    "What documents do I need to submit?",
    "Please explain the dorm application process step by step.",
    "What should I prepare for dorm check-in?",
    "What is the inquiry phone number?"
]

ENGLISH_SEARCH_MAP = {
    "When is the dorm application period?": "입사신청기간 모집일정 신청기간",
    "When is the admission result announcement?": "합격자 발표일 발표일정",
    "When is the dorm fee payment period?": "생활관비 납부기간 납부일정",
    "What documents do I need to submit?": "제출서류 증빙서류 준비물",
    "Please explain the dorm application process step by step.": "입사절차 입사신청 절차 입사등록 절차",
    "What should I prepare for dorm check-in?": "입사등록 준비물 제출서류 결핵검진결과표",
    "What is the inquiry phone number?": "문의 전화번호 연락처 문의처 행정팀 사감실 02-"
}
...
def get_auto_faq_questions(limit=9, min_count=2):
    stats = get_question_stats()
    faq = [q for q, count in stats if count >= min_count]
    return faq[:limit]
...
def build_quick_questions(answer_language="한국어"):
    top_logged_questions = get_auto_faq_questions(limit=9, min_count=2)
...
with st.sidebar:
    ...
    default_language_index = 1 if selected_user_type == "외국인 재학생" else 0
    ...
    st.caption(f"현재 선택: {selected_user_type} · {answer_language}")
...
# -----------------------------
if answer_language == "English":
    st.info(f"Selected user type: {selected_user_type} · Response language: {answer_language}")
    st.caption(f"Registered PDF pages: {len(all_pages)} · Searchable documents: {len(docs)}")
else:
    st.info(f"현재 선택한 사용자 유형: {selected_user_type} · 답변 언어: {answer_language}")
...
if answer_language == "English":
    ...
    st.caption("Click a button to ask instantly. Frequently asked questions are automatically reflected.")
else:
...
for msg in st.session_state.chat_history:
    with st.chat_message(msg["role"]):
        ...
        if msg.get("user_type"):
            if msg_lang == "English":
                st.caption(f"User type: {msg['user_type']}")
            else:
...
if prompt:
    ...
    if not filtered_docs:
        if answer_language == "English":
            answer = f"No documents were found for the selected category ({selected_user_type})."
        else:
        ...
        with st.chat_message("assistant"):
            ...
            if answer_language == "English":
                st.caption(f"User type: {selected_user_type}")
            ...
            with st.expander("View referenced files/pages" if answer_language == "English" else "참고한 파일/페이지 보기"):
                for item in selected_results:
                    ...
                    if answer_language == "English":
                        st.markdown(
                            f"**Type: {item['user_type']} | File: {item['source_file']} | "
